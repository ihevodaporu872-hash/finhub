...
-- Динамический поиск project_id по имени проекта
...
select
  ...
  '2025-05-01'::date,
  '{
    "contract_name": "Договор генподряда №К14",
    "contract_object": "ЖК PRIMAVERA квартал Bellini. К14",
    "contract_amount": 15800000000,
    "price_type": "fixed",
    "nds_rate": 20,
  ...
  '{
    ...
    "preferential_advance_pct": 10,
    "preferential_advance_bank": "ВТБ",
  ...
  '{
    "savings_gp_pct": 20,
    "savings_customer_pct": 80,
    "savings_customer_init_gp_pct": 0,
    "savings_customer_init_pct": 100,
    "price_revision_threshold_pct": 10,
    "price_revision_appendix": "Приложение №2.1",
    "insurance_go_amount": 180000000,
    "opex_items": [
      {"title": "Комиссии за ведение счетов", "description": "Счета в ВТБ"},
      {"title": "Выпуск БГ", "description": "На авансы и ГО"},
      {"title": "Страхование ГО", "description": "Полис на 180 000 000 ₽"},
      {"title": "Содержание площадки", "description": "Коммуналка, ЧОП, видеонаблюдение, СКУД FaceID"}
    ]
  }'::jsonb,
  '{
    "penalties": [
      {"violation": "Просрочка промежуточных сроков СМР", "rate": 200000, "unit": "за каждый день"},
      {"violation": "Просрочка окончания СМР или получения ЗОС", "rate": 250000, "unit": "за каждый день"},
      {"violation": "Просрочка передачи квартир дольщикам", "rate": 1580000, "unit": "за каждый день"},
      {"violation": "Задержка устранения дефектов по предписаниям", "rate": 50000, "unit": "за каждый день"},
      {"violation": "Смена контроля над ГП без согласования", "rate": 1000000, "unit": "за каждый случай"}
    ],
    "customer_penalty_rate_pct": 0.05,
    "customer_penalty_start_day": 10
  }'::jsonb
from projects p
where p.name ilike '%PRIMAVERA%' or p.name ilike '%Bellini%' or p.name ilike '%К14%'
limit 1;
