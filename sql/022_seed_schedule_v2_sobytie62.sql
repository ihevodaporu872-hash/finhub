...
BEGIN
  ...
  -- Коммерческие затраты (из договора)
  INSERT INTO schedule_v2_categories (project_id, name, cost_group, sort_order, volume, unit, price_per_unit, cost_materials, cost_labor, cost_sub_materials, cost_sub_labor, total)
  VALUES
    (v_project_id, 'Проектные работы', 'commercial', 1, 0, '', 0, 0, 0, 183212280, 0, 183212280),
    (v_project_id, 'Генподрядные услуги (12% от 2чЦСР)', 'commercial', 2, 0, '', 0, 0, 0, 0, 306921115.84, 306921115.84),
    (v_project_id, 'Прочие коммерческие расходы', 'commercial', 3, 0, '', 0, 0, 0, 103266514.69, 0, 103266514.69)
  ON CONFLICT (project_id, name, cost_group) DO UPDATE SET
    sort_order = EXCLUDED.sort_order,
    cost_materials = EXCLUDED.cost_materials,
    cost_labor = EXCLUDED.cost_labor,
